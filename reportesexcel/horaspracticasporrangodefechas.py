...
from sga.forms import HorasDictadasxFechasForm,RangoFacturasForm
...
def view(request):
    try:
        if request.method=='POST':
            ...
            if action  =='generar':
                try:
                    ...
                    if request.POST['profesor']!='':
                        ...
                        prof = LeccionGrupo.objects.filter(materia__nivel__periodo__activo=True,profesor=docente,fecha__gte=inicio,fecha__lte=fin,turno__practica=True).order_by('materia').distinct().values('materia')
                        doc=True
                    else:
                        prof = LeccionGrupo.objects.filter(materia__nivel__periodo__activo=True,fecha__gte=inicio,fecha__lte=fin,turno__practica=True).distinct().order_by('profesor').values('profesor')
                    ...
                    ws.write_merge(1, 1,0,m, 'LISTADO CLASES PRACTICAS POR RANGO DE FECHAS', titulo2)
                    ws.write(3, 0, 'DESDE:', titulo)
                    ws.write(3, 1, str(inicio), titulo)
                    ws.write(4, 0, 'HASTA:', titulo)
                    ws.write(4, 1, str(fin), titulo)
                    cont =4
                    detalle = 4
                    c=1
                    cont = cont+2
                    ...
                    nombre ='horaspracticasxrangofechas'+str(datetime.now()).replace(" ","").replace(".","").replace(":","")+'.xls'
                ...
                try:
                    ...
                    pdf.text(1,35,"DESDE: "  + str(inicio))
                    pdf.ln(5)
                    pdf.text(1,40,"HASTA: "  + str(fin))
                    pdf.ln(5)
                    ...
                    d = datetime.now()
                    pdfname = 'horaspracticasxrangofechas' + '_' + d.strftime('%Y%m%d_%H%M%S') + '.pdf'
            ...
            data = {'title': 'Horas Materias Practicas por Rango de Fechas '}
            addUserData(request,data)
            if ReporteExcel.objects.filter(activo=True,vista=request.path[1:]).exists():
                reportes = ReporteExcel.objects.filter(activo=True,vista=request.path[1:]).exists()
                data['reportes'] = reportes
                data['generarform']=HorasDictadasxFechasForm(initial={'inicio':datetime.now().date(),'fin':datetime.now().date()})
                return render(request ,"reportesexcel/horaspracticasporrangodefechas.html" ,  data)
