        ...
        if request.method=='POST':
            ...
            if action :
                ...
                try:
                    ...
                    ws.write_merge(1, 1,0,m+2, 'INFORMACION DE ESTUDIANTES POR CARRERA', titulo2)
                    ...
                    matriculados = Matricula.objects.filter(inscripcion__persona__usuario__is_active=True,nivel__cerrado=False,nivel__carrera=carrera).order_by('inscripcion__persona__apellido1','inscripcion__persona__apellido2')
            ...
            data = {'title': 'Informacion de Estudiantes por Carrera'}
            addUserData(request,data)
            if ReporteExcel.objects.filter(activo=True,vista=request.path[1:]).exists():
                reportes = ReporteExcel.objects.filter(activo=True,vista=request.path[1:]).exists()
                data['reportes'] = reportes
                data['generarform']=MatriculadosporCarreraExcelForm()
            return render(request ,"reportesexcel/informacion_estudiantes.html" ,  data)
