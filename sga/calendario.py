#Embedded file name: ./calendario.py
...
def view(request):
    if request.method == 'POST':
        ...
        # actividades = Actividad.objects.filter(Q(inicio__lte=fechainicio) & Q(fin__gte=fechainicio) | Q(inicio__lte=fechafin) & Q(fin__gte=fechafin) | Q(inicio__lte=fechainicio) & Q(fin__gte=fechafin) | Q(inicio__gte=fechainicio) & Q(fin__lte=fechafin), periodo=data['periodo']).order_by('id')
        ...
        for i in lista.items():
            ...
            if comienzo and fin is False:
                ...
                s_dia += 1
                lista.update(dia)
                # actividadesdias = Actividad.objects.filter(Q(inicio__lte=fecha) & Q(fin__gte=fecha), periodo=data['periodo']).order_by('id')
        ...
        return render(request ,"calendario/calendariobs.html" ,  data)
