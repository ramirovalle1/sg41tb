...
def validarmedioteenterastes(medioteenterastes):
    idmedioenterastes=0
    for x in TipoAnuncio.objects.filter(id__in=[25,13,27,28,29,30,31,32,33,34],activo=True).order_by("descripcion"):
        if medioteenterastes.id==x.id:
            idmedioenterastes=x.id
    return idmedioenterastes
def view(request):
    if request.method == 'POST':
        ...
        if action == 'validaridentificacion':
            try:
                ...
                if not SolicitudPostulacionBeca.objects.filter(identificacion=str(request.POST['cedu']).strip(),empresaconvenio=empresaconvenio).exists():
                    return HttpResponse(json.dumps({'result': 'bad', 'message': str("Usted no se encuentra registrado en la postulación")}),
                ...
                data['datospersona'] = [{"cedula": datospersona.identificacion,
                                         ...
                                         "medionterastes":validarmedioteenterastes(datospersona.anuncio) if datospersona.anuncio else '0'
        ...
        return render(request,"solicitudpostulacionbeca/solicitudpostulacionbecadjudicadabs.html", data)