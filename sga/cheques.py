...
from sga.models import Factura, PagoCheque, ChequeProtestado, InscripcionFlags, TipoOtroRubro, Rubro, RubroOtro, RubroNotaDebito
...
def view(request):
    if request.method=='POST':
        ...
        if action=='protestar':
            try:
                ...
                if f.is_valid():
                    ...
                    inscripcion = None
                    fichamedica = None
                    if cheque.pagos.all()[0].rubro.inscripcion:
                        inscripcion = cheque.pagos.all()[0].rubro.inscripcion
                    if cheque.pagos.all()[0].rubro.fichamedica:
                        fichamedica = cheque.pagos.all()[0].rubro.fichamedica
                    ...
                    rubro = Rubro(fecha=hoy, valor=cheque.valor + banco.tasaprotesto,
                                  inscripcion=inscripcion,fichamedica=fichamedica, cancelado=False, fechavence=vence)
                    rubro.save()
        ...
        data = {'title': 'Listado de Cheques'}
        addUserData(request,data)
        if 'action' in request.GET:
            action = request.GET['action']
            if action=='protestar':
                data['title'] = 'Protestar Cheques'
                cheque = PagoCheque.objects.get(pk=request.GET['id'])
                data['cheque'] = cheque
                data['form'] = ChequeProtestadoForm()
                return render(request ,"cheques/protestarbs.html" ,  data)
            elif action=='fechacobro':
                data['title'] = 'Editar la Fecha de Cobro del Cheque'
                if 'error' in request.GET:
                    data['error'] = request.GET['error']
                cheque = PagoCheque.objects.get(pk=request.GET['id'])
                initial = model_to_dict(cheque)
                data['cheque'] = cheque
                data['form'] = ChequeFechaCobroForm(initial=initial)
                return render(request ,"cheques/fechacobro.html" ,  data)
        else:
            search = None
            if 's' in request.GET:
                search = request.GET['s']
            if search:
                cheques = PagoCheque.objects.filter(Q(numero__icontains=search) | Q(banco__nombre__icontains=search) | Q(valor__icontains=search) | Q(emite__icontains=search)).order_by('fecha')
            else:
                cheques = PagoCheque.objects.all().order_by('-fecha','fechacobro')
            paging = MiPaginador(cheques, 30)
            p = 1
            try:
            ...
            return render(request ,"cheques/chequesbs.html" ,  data)
