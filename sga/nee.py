...
def view(request):
    if request.method=='POST':
        ...
        data = {'title': 'Estudiante NEE'}
        addUserData(request,data)
        if 'action' in request.GET:
            action = request.GET['action']
            if action=='detalles':
                data['title'] = 'Detalles NEE'
                ...
                return render(request ,"nee/detalle.html" ,  data)
        else:
            search = None
            ...
            paging = MiPaginador(perfiles, 300)
            p = 1
            try:
            ...
            return render(request ,"nee/nee.html" ,  data)
