...
from sga.models import RetiradoMatricula, DetalleRetiradoMatricula, Persona, convertir_fecha, MatriculaLiberada, Matricula
import json
        ...
        try:
            if 'action' in request.GET:
                ...
                if action == 'ver':
                    ...
                    data['detalle'] = MatriculaLiberada.objects.filter(matricula=matricula)
                    return render(request ,"liberarmatricula/detalle.html" ,  data)
                if action == 'retirar':
                    data={}
            ...
            search = None
            ids = MatriculaLiberada.objects.filter().distinct('matricula').order_by('matricula').values('matricula')
            matriculas = Matricula.objects.filter(id__in=ids).order_by('fecha')
            if 's' in request.GET:
            ...
            if search:
                ...
                if len(ss)==1:
                    ...
                    matriculas = matriculas.filter(Q(inscripcion__persona__apellido1__icontains=ss[0]) & Q(inscripcion__persona__apellido2__icontains=ss[1])).order_by( 'inscripcion__persona__apellido1','inscripcion__persona__apellido2','inscripcion__persona__nombres')
            ...
            return render(request ,"liberarmatricula/registros.html" ,  data)
        except Exception as ex:
