...
from sga.models import TipoIdentificacion, SolicitudPostulacionBeca,  Canton, Pais, Provincia, \
    elimina_tildes, Sexo, Parroquia,  DocumentosPostulacionBeca, \
     EmpresaConvenio,  ConvenioCarrera, Modalidad, TipoAnuncio
    ...
    if request.method == 'POST':
        ...
        if action == 'agregarsolicitud':
            try:
                ...
                empresaconvenio = EmpresaConvenio.objects.filter(id=41,activapostulacion=True).first()
        ...
        if action == 'validaridentificacion':
            try:
                ...
                empresaconvenio=EmpresaConvenio.objects.get(id=41,activapostulacion=True)
        ...
        if action == 'buscarcantones':
            ...
            try:
                ...
                if ConvenioCarrera.objects.filter(empresaconvenio__id=41,modalidad__id=int(request.POST['idmodalidad'])).exists():
                    for a in ConvenioCarrera.objects.filter(empresaconvenio__id=41,modalidad__id=int(request.POST['idmodalidad'])).order_by("carrera__nombre"):
        ...
        data['listsexobecario'] = Sexo.objects.filter()
        data['listmodalidad'] = Modalidad.objects.filter(id__in=[4,5])
        ...
        return render(request,"solicitudpostulacionbeca/solicitudpostulaciongeneralbs.html", data)