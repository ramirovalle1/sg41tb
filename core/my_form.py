    ...
    * max_upload_size - a number indicating the maximum file size allowed for upload.
    ...
    def clean(self, *args, **kwargs):
        ...
        if upload:
            ...
            if size == 0:
                raise forms.ValidationError(f'El tamaño máximo permitido no valido {self.max_upload_size} bytes.')
            elif size > self.max_upload_size:
                raise forms.ValidationError(f'El archivo supera el tamaño máximo permitido {self.max_upload_size} bytes.')
            elif ext not in self.ext_whitelist:
                raise forms.ValidationError("Tipo de fichero no permitido!")
...
class MY_Form(forms.Form):
    ...
    def toArray(self):
        ...
        for field in self.fields:
            ...
            if isinstance(self.fields[field], forms.BooleanField):
                ...
                if field in self.data:
                    value = self.data[field] if self.data[field] else ''
                elif field in self.initial:
                    value = self.initial[field]
                else:
                    value = ''
    ...
    def clean(self, *args, **kwargs):
        cleaned_data = super(MY_Form, self).clean(*args, **kwargs)
